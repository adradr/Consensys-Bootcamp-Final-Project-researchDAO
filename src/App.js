...
class App extends Component {
  ...
  componentDidMount = async () => {
    try {
      ...
      this.memberItems = this.state.members.map((item) =>
      <li>{item}</li>
      );
      ...
      this.proposalItems = this.state.proposalQueue.map((item) =>
      <li>#{item.proposalIndex}: {item.title}, {item.proposalApplicant}, {item.documentationAddress}</li>
      );
